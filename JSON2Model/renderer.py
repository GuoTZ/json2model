...
class TemplateYYModelRenderer(object):
    """Template Renderer
    """

    def __init__(self, properties_h, properties_m, output_dir='output'):
        basepath = os.path.dirname(__file__)
        h_file = os.path.abspath(os.path.join(basepath, 'templates', 'yymodel.h'))
        m_file = os.path.abspath(os.path.join(basepath, 'templates', 'yymodel.m'))
        self.properties = {
            'h': properties_h,
            'm': properties_m,
        }
